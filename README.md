...
<br>结合以ifc+gis+iot为核心的微服务,实现了自己所期望的内容.
...
<br>调用后通过指令处理函数将指令映射为请求参数,
<br>并提交到对应的服务执行.
<br>指令模块的返回除了支持文本形式的返回外,
<br>还支持音视频,模型等形式等形式的返回.
<br>音视频模型等相较于文本更易理解,
<br>调用时可以额外调用'渲染函数'向调用方提供更友好的数据渲染.
<br>同时指令模块中的指令既可以通过'方案应用模块'被llm调用也可以单独被服务集成
![指令模块附图](./document/image/daytime_agent_di2_instructions.png)
<br>当下随着越来越多的ai辅助编程工具,
<br>个体也可以更容易的生成做为微服务的指令.
<br>亦可以如现实协作一般互相通过api端点开放能力.
<br>通过指令实现互助,让彼此生活可以更舒服.
#### 指令示例
