...
<br>该项目通过结合DDD+DSL进行LLMS的'形式化逻辑推理';
<br>通过LLMS+微服务完成'形式化逻辑推理'的'推理结果'到'数据处理结果'的转化;
<br>再用ifc+gis+iot完成'数据处理结果'到'物理现实的落地'.
<br>最终实现'AI以人类能理解的方式自主使用相关工具解决现实世界的问题'

----------
<br>当下咨询AI如何解决一个复杂问题时,基本上能得到一个相对合适的解决方案.
<br>但是我需要的不是解决方案,而是需要'它'直接帮我解决'现实问题'.
<br>为了让自己有更多的时间用来生活,
<br>我通过整合自身的资源和技能实现了AI生成解决方案在现实场景中的自动实施.
<br>自有服务运行了一段时间后,我觉得可以有更多的人享受这种便利.
<br>所以把主要'逻辑'和'代码片段'放到项目中供大家参考.
## 模块构成
<br>项目主要由以下四个模块构成：
...
### 方案应用模块
<br>方案应用模块可对'结构化自然文本'(符合预期结构句法的自然语言段落)进行推理.
<br>推理分为线性推理和节点推理,线性推理通过正则获取'结构化自然文本'中的逻辑,
<br>节点推理通过antlr4+neo4j实现对'结构化自然文本'中的逻辑,
#### 方案文本示例
...
#### 模块示例
##### 穿什么
<br>其中在该示例应用中未实际调用'虚拟换装'及使用私域'资源'<br>
![明天威海穿搭穿什么](https://github.com/weihai-limh/daytime_agent/blob/main/document/video/agent_test1.gif)
<br>其数据链路如下:
```
//用户通过代理模块输入：
{'prompt':'明天威海穿搭推荐'}
