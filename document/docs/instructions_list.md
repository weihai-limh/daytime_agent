...
### 深度转换
深度转换服务根据指令中的图像url及深度转换处理对应的图像
深度转换有两种模态分别为
该服务主要应用''
指令示例
### 点云转换
点云转换服务根据指令中的图像url及点云转换处理对应的图像
点云转换有两种模态分别为
该服务主要应用''
指令示例
### 视频转序列图片
视频转序列图片服务根据指令中的视频url及参数处理对应的视频
该服务主要应用''完成对视频的处理
类似指令还包括'图片序列转视频','格式转化','裁剪视频起始时长','裁剪视频起始时长'
'裁剪视频起始终止','获取指定视频帧','压缩视频','更改视频分辨率'
'视频提取音频','视频去音频','视频加音频'
指令示例
...
### 中译英
中译英服务从指令中提取'待翻译的文本'然后将其翻译为英文
中译英服务通过封装在线的翻译服务完成翻译
### 语言转化
...
指令示例
### 获取资源
获取资源服务从指令中提取要查询的资源参数进而查询资源
资源分为公共资源和个人资源,个人资源比公共资源多一个'用户代称'的查询参数
公共资源仅需'资源类型','资源标签'两个参数
'资源标签'包含'资源实例的名称','资源标签'
指令示例
### 语音合成
发送邮件服务通过本地部署的邮件服务器
通过改服务向目标邮箱发送对应文件
...
## 语义转化服务
### 建筑模型转建筑语义
建筑模型转建筑语义服务将建筑模型(.ifc)转化为符合建筑语义的'数字孪生场景'
服务通过'IfcOpenShell'完成IFC文件的预处理,完成语义数据和几何数据的提取,
后续通过按'数字孪生场景的建筑模板'转化对应的属性
指令示例
### 建筑模型转空间语义
建筑模型转空间语义服务在'建筑模型转建筑语义'的基础上完成对应的转化后
后续通过按'数字孪生场景的空间模板'转化对应的属性
指令示例
### 地理信息数据转geojson
地理信息数据转geojson服务将(.shp,.gpx)格式的文件转化为geojson结构的数据
指令示例
### 文本转知识图谱
文本转知识图谱将通过解析指定格式的(.txt)或文本内容,
将解析的内容转化为'图结构',通过绘制图表的'知识图谱'指令生成对应图表
指令示例
### 文本转ppt
文本转ppt服务将通过解析指定格式的(.txt)或文本内容,
将解析的内容通过'python-pptx'生成对应的(.ppt)文件
指令示例
...
机器操作详见XXX
### 空间移动
空间移动服务接受一个'导航路径'和一个机器名称
如果该机器在'导航路径'所在的'数字孪生场景'中
则让该机器按'导航路径'在'数字孪生场景'对应的现实空间中行驶
'导航路径'对应的数据为指令'资产导航'或'建筑导航'生成的室内导航数据
指令示例
### 工作流
机器操作的工作流服务接受一个'动作序列'和一个机器名称
服务根据机器名称将'动作序列'写入到机器上,机器按序列执行动作
指令示例
### 机器执行
机器操作的工作流服务接受一个'机器执行动作'和一个机器名称
服务将'机器执行动作'转化为机器的执行指令并推送到机器供机器执行
指令示例

## 地理空间服务
地理空间服务服务通过指令返回对应的geojson信息
地理空间对应的数据源来自在线地图服务(百度,高德,天地图)及OSM
除公域数据外个人数据通过'地理信息数据转geojson'将各类数据转化为geojson
拓扑计算等一般应用通过QGIS进行对应的数据处理,空间句法等应用结合neo4j完成
### 网址查询
网址查询服务通过传入的网址或ip返回对应城市的所在经纬度
并通过h3生成城市范围的geojson
指令示例
### 经纬度查询(地址查询)
经纬度查询服务及地址查询查询通过传入的经纬度及坐标和辅助参数进行处理
服务根据参数以geojson的格式返回热点的空间信息
指令示例
### 静态地图
...
指令示例
### 位置查找(位置连线,行政范围)
位置查找服务与位置连线服务,行政范围服务相类似
该类服务是'经纬度查询','地址查询'的升级应用
服务通过'经纬度查询','地址查询'获得经纬度信息后结合需要生成新的geojson
指令示例
### 室外导航
